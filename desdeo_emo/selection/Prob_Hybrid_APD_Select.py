...
class Prob_Hybrid_APD_Select(SelectionBase):
    ...
    def do(self, pop: Population, vectors: ReferenceVectors) -> List[int]:
        ...
        for i in range(0, len(vectors.values)):
            ...
            if len(sub_population_fitness_prob) > 0:
                ...
                pwrong.pdf_list = {}
                pwrong.ecdf_list = {}
                pwrong.compute_pdf(apd_prob)
                #pwrong.plt_density(apd)
                pwrong.compute_rank_vectorized2()
                rank_apd_prob = pwrong.rank_prob_wrong
                #print(rank_apd)
                apd_elites = apd_prob[0,np.where(rank_apd_prob[0,:]<0),:]
                if np.size(apd_elites) >= 1:
                    rank_apd_prob = np.mean(apd_elites, axis=2)
