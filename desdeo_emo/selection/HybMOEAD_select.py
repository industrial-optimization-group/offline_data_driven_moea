...
class HybMOEAD_select(SelectionBase):
    ...
    def pbi_dist(self, objective_values, weights, ideal_point, pwrong_f_samples, theta = 5):
    ...
    def _evaluate_SF_dist(self, neighborhood, weights, ideal_point, pwrong):
        if self.SF_type == "TCH":
            SF_values = np.array(list(map(self.tchebycheff, neighborhood, weights, ideal_point)))
            return SF_values
        elif self.SF_type == "PBI":
            SF_values = np.array(list(map(self.pbi_dist, neighborhood, weights, ideal_point, pwrong.f_samples)))
            return SF_values
