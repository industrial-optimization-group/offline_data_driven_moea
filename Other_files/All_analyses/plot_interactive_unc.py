...
def animate_init_(data: Union[np.ndarray, pd.DataFrame, list],
                  data_unc: Union[np.ndarray, pd.DataFrame, list],
    ...
    numobj = data.shape[1]
    if numobj == 2:
        figure = animate_2d_init_(data,
                                  data_unc,
                                  preference,
                                  vectors,
                                  filename)
    elif numobj == 3:
        figure = animate_3d_init_(data,
                                  data_unc,
                                  preference,
                                  vectors,
                                  filename)
    elif numobj >= 4:
        figure = animate_parallel_coords_init_(data,
                                               data_unc,
                                               preference,
                                               vectors,
                                               filename)
...
def animate_next_(
    ...
    numobj = data.shape[1]
    if numobj == 2:
        figure = animate_2d_next_(data,
                                  data_unc,
                                  preference,
                                  vectors,
                                  generation)
    elif numobj == 3:
        figure = animate_3d_next_(data,
                                  data_unc,
                                  preference,
                                  vectors,
                                  generation)
    elif numobj >= 4:
        figure = animate_parallel_coords_next_(data,
                                               data_unc,
                                               preference,
                                               vectors,
                                               generation)
...
def animate_2d_init_(
    ...
    figure = {"data": [], "data_unc": [], "preference": [], "vectors": [], "layout": {}, "frames": []}
...
def animate_parallel_coords_init_(
    ...
    uncertainty = pd.DataFrame(data_unc)
    preference = pd.DataFrame(preference)
    vectors = pd.DataFrame(vectors)
    avg_uncertainty = pd.DataFrame(np.mean(data_unc, axis=1))
    figure["layout"]["hovermode"] = "closest"
    figure["layout"]["sliders"] = {
        "args": ["transition", {"duration": 400, "easing": "cubic-in-out"}],
        "initialValue": "1952",
    ...
    line = dict(
        ...
        colorscale="Viridis",
        showscale=True,
        cmin=min(avg_uncertainty),
        cmax=max(avg_uncertainty),
    )

    data_dict = go.Parcoords(line=line, dimensions=dimensions)
    figure["data"].append(data_dict)
    plot(figure, filename=filename)
    animate_parallel_coords_next_(data, data_unc, preference, vectors, figure, filename, 0)
    return figure
